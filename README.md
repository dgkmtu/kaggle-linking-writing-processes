...
nb09
- 写経の続き
- class化されているので、少し複雑だけど、ここまで複雑な処理はなさそうな印象
- これまでの処理とどう違うのかは、比較する必要があるな。
- 問題文を全て頭に入れるために、ノートに文章を書こう。
...
### 20231126
nb10
- nb10を提出すると、スコアは2023年11月26日時点で0.582の56位だった。
...
nb11
- 特徴量の記載の継続。
- 今日はエッセイの段落ごとの特徴量抽出とキーロガー情報の特徴量抽出について記載するところだね。
- 二つの理解と記載は終わった。次は、残り二つある特徴量の作成について理解すれば特徴量作成は完了するね。
- ハイパーパラメータ調整のcodeを発見した、追加で記載しておこうかな。

### 20231201
nb11
- ハイパーパラメータの調整を動かしてみる。ここで得られたパラメータを適用してsubmitしてみよう

### 20231204
nb12
- 現状のnotebookは２つのモデルを統合して作成している。
- 前者のモデルのパラメータ調整はできているけど、後者のモデル調整はできていなかったので、nb12で作成しよう。
- まずは徹底的に模倣するところから！

### 20231206
nb12
- パラメータ調整の続き。後者のモデルのパラメータ調整は難しいね。大分スコアが悪くなってしまう。
- あんまり動かしても無意味なのかと感じたね。

### 20231208
nb12
- なんかnotebookが更新されてスコアが動いたので、早速参加した。
- スコア的にはあまり動かないけど、このnotebookをensembleすると、さらにスコアが伸びそうなので統合してみる。

nb13
- nb13を新規作成してnb10とnb12を統合してみた。
- スコアが0.579にアップして銀メダル圏内に！
- まだ、巻き返しが来る可能性もあるので、パラメータの調整を引き続きやっていこう。
- 中々上手くいかないんだけどね〜